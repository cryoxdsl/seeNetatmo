...
{
    $copyright = '© ' . date('Y') . ' ' . app_name();
    $thirdPartyLinks = '<a href="https://open-meteo.com/" target="_blank" rel="noopener noreferrer">' . h(t('site.forecast_source_openmeteo')) . '</a>, <a href="https://www.met.no/" target="_blank" rel="noopener noreferrer">' . h(t('site.forecast_source_metno')) . '</a>';
    echo '</main><footer class="foot"><div class="wrap">' . h($copyright) . ' | ' . h(t('footer.license')) . ' | ' . h(t('footer.contact')) . ': <a href="mailto:' . h(contact_email()) . '">' . h(contact_email()) . '</a> | <a href="/terms.php">' . h(t('footer.terms')) . '</a> | ' . h(t('footer.third_party')) . ': ' . $thirdPartyLinks . ' | <a href="https://github.com/cryoxdsl/seeNetatmo" target="_blank" rel="noopener noreferrer">Git</a></div></footer>';
    ...
    echo '<script>(function(){if(window.__m13ClientErrInit){return;}window.__m13ClientErrInit=true;var EP="/front-log.php";var VID_KEY="m13_vid";var SID_KEY="m13_sid";var sent=0;var lastTs=0;var seen={};var norm=function(v,max){v=(v==null?"":String(v));return v.slice(0,max);};var send=function(level,message,extra){var sig=norm(level,10)+"|"+norm(message,180)+"|"+norm(extra&&extra.url?extra.url:(location.pathname||"/"),120)+"|"+(extra&&extra.line?extra.line:0);var now=Date.now();if(seen[sig]){return;}if(sent>=25){return;}if(now-lastTs<250){return;}seen[sig]=1;sent++;lastTs=now;var payload={level:(level==="warning"?"warning":"error"),message:norm(message||"Client error",300),kind:norm(extra&&extra.kind?extra.kind:"js",40),path:norm(location.pathname||"/",255),url:norm(extra&&extra.url?extra.url:(location.href||""),255),line:parseInt(extra&&extra.line?extra.line:0,10)||0,col:parseInt(extra&&extra.col?extra.col:0,10)||0,stack:norm(extra&&extra.stack?extra.stack:"",1200),referrer:norm(document.referrer||"",255),visitor_id:norm((function(){try{return localStorage.getItem(VID_KEY)||"";}catch(_e){return"";}})(),64),session_token:norm((function(){try{return sessionStorage.getItem(SID_KEY)||"";}catch(_e){return"";}})(),32),lang:norm(document.documentElement.lang||"",16)};try{if(navigator.sendBeacon){var blob=new Blob([JSON.stringify(payload)],{type:"application/json"});navigator.sendBeacon(EP,blob);return;}}catch(_e){}fetch(EP,{method:"POST",credentials:"same-origin",cache:"no-store",keepalive:true,headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)}).catch(function(){});};window.addEventListener("error",function(ev){if(ev&&ev.target&&ev.target!==window){var t=ev.target;var u=(t.currentSrc||t.src||t.href||"");send("warning","Resource load failed",{kind:"resource",url:u});return;}send("error",(ev&&ev.message)||"JS error",{kind:"error",url:ev&&ev.filename?ev.filename:location.href,line:ev&&ev.lineno?ev.lineno:0,col:ev&&ev.colno?ev.colno:0,stack:ev&&ev.error&&ev.error.stack?String(ev.error.stack):""});},true);window.addEventListener("unhandledrejection",function(ev){var reason=ev&&typeof ev.reason!=="undefined"?ev.reason:"Unhandled rejection";var msg=(reason&&reason.message)?reason.message:String(reason);var stack=(reason&&reason.stack)?String(reason.stack):"";send("error",msg,{kind:"unhandledrejection",url:location.href,stack:stack});});})();</script>';
    echo '</body></html>';
}
